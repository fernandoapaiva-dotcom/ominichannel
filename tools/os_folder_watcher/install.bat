...
echo ============================================
echo  Instalador do Vigia de Pasta - Ominichannel
echo ============================================
echo.
...
:: 1. config.json precisa existir e estar preenchido (chave de API + pastas reais)
if not exist "config.json" (
    ...
    echo e os caminhos das pastas antes de instalar.
    pause
    exit /b 1
)

:: 2. Roda como administrador? Se sim, instala pra TODOS os operadores da estacao.
::    Se nao, instala so pro usuario atual (ainda funciona, so nao cobre outros logins).
net session >nul 2>&1
if %errorlevel% neq 0 (
...
:: 3. Verifica se o Python ja esta instalado; se nao, baixa e instala silenciosamente
...
:: 4. Cria um ambiente Python isolado so pra este programa (nao mexe no resto da estacao)
if not exist "venv" (
    echo Criando ambiente Python isolado...
    python -m venv venv
)

:: 5. Instala as dependencias dentro desse ambiente isolado
...
:: 6. Gera o iniciador silencioso (sem janela preta abrindo a cada login)
echo Criando o iniciador automatico...
(
echo Set WshShell = CreateObject^("WScript.Shell"^)
echo WshShell.CurrentDirectory = "%SCRIPT_DIR%"
echo WshShell.Run """%SCRIPT_DIR%venv\Scripts\pythonw.exe"" ""%SCRIPT_DIR%watcher.py""", 0, False
) > "%SCRIPT_DIR%run_watcher.vbs"

:: 7. Copia o iniciador para a pasta de Inicializacao do Windows - roda sozinho a cada login
copy /Y "%SCRIPT_DIR%run_watcher.vbs" "%STARTUP_DIR%\OminichannelWatcher.vbs" >nul
if %errorlevel% neq 0 (
    echo [ERRO] Nao foi possivel copiar para a pasta de Inicializacao: %STARTUP_DIR%
    pause
    exit /b 1
)
echo Instalado em: %STARTUP_DIR%\OminichannelWatcher.vbs
echo.

:: 8. Ja inicia agora tambem, sem precisar reiniciar/logar de novo pra testar
echo Iniciando o vigia agora para teste...
wscript.exe "%SCRIPT_DIR%run_watcher.vbs"
timeout /t 3 >nul
...
echo ============================================
echo O vigia de pasta vai iniciar sozinho sempre que alguem fizer login
echo nesta estacao. Confira o arquivo watcher.log nesta pasta para ver
echo se esta funcionando (deve aparecer "Observando [abertura]..." e
echo "Observando [orcamento]...").
echo.
pause
