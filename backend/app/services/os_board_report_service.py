...
def build_os_report_pdf(
    *,
    ...
    # ---------------------------------------------------------------- resumo (KPIs)
    story.append(Paragraph("Resumo do período", h2))
    kpis = [
        ("O.S. no filtro", str(total_os)),
        ("Finalizadas", str(len(finalizadas))),
        ("Em aberto", str(len(abertas))),
        ("Taxa de aprovação", f"{taxa_aprovacao:.0f}%" if taxa_aprovacao is not None else "—"),
    ...
    tecnicos = sorted({o["tecnico_label"] for o in orders})
    ...
    for tec in tecnicos:
        tec_orders = [o for o in orders if o["tecnico_label"] == tec]
    ...
    if com_valor:
        ...
        for tec in tecnicos:
            tec_valores = [o["valor_total"] for o in orders if o["tecnico_label"] == tec and o.get("valor_total") is not None]
            if not tec_valores:
                continue
        ...
        ft = Table(fin_rows, colWidths=[doc.width * 0.5, doc.width * 0.2, doc.width * 0.3])
        ...
        # ---------------------------------------------------------------- forma de pagamento
        story.append(Paragraph("Total por forma de pagamento", h2))
        ...
        pt = Table(pag_rows, colWidths=[doc.width * 0.42, doc.width * 0.16, doc.width * 0.26, doc.width * 0.16])
    ...
    story.append(Paragraph("Distribuição por tipo de O.S.", h2))
    ...
    tt = Table(tipo_rows, colWidths=[doc.width * 0.55, doc.width * 0.25, doc.width * 0.2])
    ...
    # ---------------------------------------------------------------- ranking de técnicos
    story.append(Paragraph("Ranking de técnicos (finalizadas no período)", h2))
    ranking = sorted(
        ((tec, sum(1 for o in orders if o["tecnico_label"] == tec and o["stage"] == "finalizada")) for tec in tecnicos),
        key=lambda kv: -kv[1]
    )
    rank_rows = [["#", "Técnico", "Finalizadas"]] + [[str(i + 1), tec, str(n)] for i, (tec, n) in enumerate(ranking)]
    rt = Table(rank_rows, colWidths=[12 * mm, doc.width - 12 * mm - 30 * mm, 30 * mm])
    ...
    if incluir_lista and orders:
        ...
        for o in listed:
            det_rows.append([
                ...
                Paragraph((o.get("cliente") or "—")[:34], cell), o["tecnico_label"],
                Paragraph((o.get("tipo_os") or "—")[:16], cell), Paragraph(o["situacao"], cell),
                _fmt_date(o.get("data_entrada")), _fmt_money(o.get("valor_total")),
                Paragraph((o.get("forma_pagamento") or "—")[:16], cell),
            ])
    ...
    story.append(Paragraph(
        ...
        "não aparecem no quadro ao vivo, mas entram neste relatório quando o filtro pedir.", note
    ))
