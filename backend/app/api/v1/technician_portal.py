...
O PIN é definido/trocado pelo admin na tela de Técnicos já existente (AdminPanel > Técnicos),
não aqui - este router só autentica quem já tem um PIN definido.
"""
import difflib
import logging
import re
...
from app.core.security import create_technician_access_token, get_current_technician, verify_password
...
async def technician_login(payload: TechnicianLoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    return TechnicianLoginResponse(access_token=token, nome=tech.nome, cargo=tech.cargo)
